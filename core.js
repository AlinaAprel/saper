...
function matrixToHtml(matrix) {
  ...
  for (let y = 0; y < matrix.length; y++) {
    ...
    for (let x = 0; x < matrix[y].length; x++) {
      ...
      if (cell.poten) {
        imgElement.src = 'assets/poten.jpeg';
        continue;
      }
      ...
      if (cell.mine) {
        imgElement.src = 'assets/bomb.svg';
        continue;
      }

      if (cell.number) {
        imgElement.src = 'assets/number' + cell.number + '.svg';
        continue;
      }
    }
...
function runOnMatrix(matrix, handler) {
  for (let y = 0; y < matrix.length; y++) {
    for (let x = 0; x < matrix[y].length; x++) {
      handler(matrix[y][x]);
    }
...
function showSpread(matrix, x, y) {
  ...
  runOnMatrix(matrix, x => x._marked = false);
